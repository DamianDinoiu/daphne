...
void innerJoinIntersection(
    ...
    bool lhsUnique = false;
    bool rhsUnique = false;

    if (lhs->properties)
        lhsUnique = lhs->properties->unique;
    if (rhs->properties)
        rhsUnique = rhs->properties->unique;

    if (numRowLhs < numRowRhs) {

        if (lhsUnique) {
             std::unordered_map<VTCol, int> hit;
             for (size_t row_idx_l = 0; row_idx_l < numRowLhs; row_idx_l++) {
                auto lValue = lCol->get(row_idx_l, 0);
                hit[lValue]= (row_idx_l);
             }
            for (size_t row_idx_r = 0; row_idx_r < numRowRhs; row_idx_r++) {
            if (auto search = hit.find(rCol->get(row_idx_r, 0)) ; search != hit.end()) {
                    lHit->push_back(search->second);
        ...
        std::unordered_map<VTCol, std::vector<int>> hit;
        for (size_t row_idx_l = 0; row_idx_l < numRowLhs; row_idx_l++) {
            auto lValue = lCol->get(row_idx_l, 0);
            hit[lValue].push_back(row_idx_l);
        }

        for (size_t row_idx_r = 0; row_idx_r < numRowRhs; row_idx_r++) {
            if (auto search = hit.find(rCol->get(row_idx_r, 0)) ; search != hit.end()) {
                for (size_t l = 0; l < search->second.size(); l++) {
                    lHit->push_back(search->second[l]);
                    rHit->push_back(row_idx_r);
                }
            }
        }   
        ...
        if (rhsUnique) {
            std::unordered_map<VTCol, int> hit;
            for (size_t row_idx_r = 0; row_idx_r < numRowRhs; row_idx_r++) {
                auto rValue = rCol->get(row_idx_r, 0);
                hit[rValue]= (row_idx_r);
            }

            for (size_t row_idx_l = 0; row_idx_l < numRowLhs; row_idx_l++) {
                if (auto search = hit.find(lCol->get(row_idx_l, 0)) ; search != hit.end()) {
                        lHit->push_back(row_idx_l);
                        rHit->push_back(search->second);
                }
            }
            return;
        }
        ...
        for (size_t row_idx_r = 0; row_idx_r < numRowRhs; row_idx_r++) {
            auto rValue = rCol->get(row_idx_r, 0);
            hit[rValue].push_back(row_idx_r);
        }
        ...
        for (size_t row_idx_l = 0; row_idx_l < numRowLhs; row_idx_l++) {
            if (auto search = hit.find(lCol->get(row_idx_l, 0)) ; search != hit.end()) {
                for (size_t l = 0; l < search->second.size(); l++) {
                    lHit->push_back(row_idx_l);
                    rHit->push_back(search->second[l]);
                }
            }
        }
    }
